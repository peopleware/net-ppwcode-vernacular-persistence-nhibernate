...
{
    /// <summary>
    ///     This code is copied from nhibernate solutions, improved by various commits (but not yet released v4.1), specially
    ///     we use this code for following bug:
    ///     <list type="bullet">
    ///         <item>
    ///             <term>https://nhibernate.jira.com/browse/NH-2931</term>
    ///         </item>
    ///     </list>
    /// </summary>
    [SuppressMessage("ReSharper", "InconsistentNaming", Justification = "Reviewed: code is from nHibernate")]
    public class ModelMapper
    {
        ...
        public ModelMapper()
            : this(new ExplicitlyDeclaredModel())
        {
        }
        ...
        {
            ClassCustomizer<TRootEntity> customizer = new ClassCustomizer<TRootEntity>(explicitDeclarationsHolder, customizerHolder);
        ...
        {
            SubclassCustomizer<TEntity> customizer = new SubclassCustomizer<TEntity>(explicitDeclarationsHolder, customizerHolder);
            customizeAction(customizer);
        }

        public void JoinedSubclass<TEntity>(Action<IJoinedSubclassMapper<TEntity>> customizeAction) where TEntity : class
        {
            JoinedSubclassCustomizer<TEntity> customizer = new JoinedSubclassCustomizer<TEntity>(explicitDeclarationsHolder, customizerHolder);
            customizeAction(customizer);
        }

        public void UnionSubclass<TEntity>(Action<IUnionSubclassMapper<TEntity>> customizeAction) where TEntity : class
        {
            UnionSubclassCustomizer<TEntity> customizer = new UnionSubclassCustomizer<TEntity>(explicitDeclarationsHolder, customizerHolder);
            customizeAction(customizer);
        }

        public void Component<TComponent>(Action<IComponentMapper<TComponent>> customizeAction) where TComponent : class
        {
            ComponentCustomizer<TComponent> customizer = new ComponentCustomizer<TComponent>(explicitDeclarationsHolder, customizerHolder);
            customizeAction(customizer);
        }
        ...
        public HbmMapping CompileMappingFor(IEnumerable<Type> types)
        {
            ...
            List<Type> typeToMap = types.Distinct()
                                        .OrderBy(x => x, new TypeHierarchyComparer())
                                        .ToList();

            string defaultAssemblyName = null;
            string defaultNamespace = null;
            Type firstType = typeToMap.FirstOrDefault();
            ...
            HbmMapping mapping = NewHbmMapping(defaultAssemblyName, defaultNamespace);
            foreach (Type type in RootClasses(typeToMap))
            {
                MapRootClass(type, mapping);
            }
            foreach (Type type in Subclasses(typeToMap))
            {
        ...
        public IEnumerable<HbmMapping> CompileMappingForEach(IEnumerable<Type> types)
        {
            ...
            foreach (Type type in RootClasses(typeToMap))
            {
                HbmMapping mapping = NewHbmMapping(type.Assembly.GetName().FullName, type.Namespace);
                MapRootClass(type, mapping);
        ...
        {
            HbmMapping hbmMapping = new HbmMapping
                                    {
        ...
        {
            UnionSubclassMapper classMapper = new UnionSubclassMapper(type, mapping);

            IEnumerable<MemberInfo> candidateProperties = null;
            if (!modelInspector.IsEntity(type.BaseType))
            {
                Type baseType = GetEntityBaseType(type);
                if (baseType != null)
                {
        ...
        {
            SubclassMapper classMapper = new SubclassMapper(type, mapping);
            IEnumerable<MemberInfo> candidateProperties = null;
            if (!modelInspector.IsEntity(type.BaseType))
            {
                Type baseType = GetEntityBaseType(type);
            ...
            customizerHolder.InvokeCustomizers(type, classMapper);
            foreach (IJoinMapper joinMapper in classMapper.JoinMappers.Values)
            {
                customizerHolder.InvokeCustomizers(type, joinMapper);
            }

            IEnumerable<string> splitGroups = modelInspector.GetPropertiesSplits(type);
            IEnumerable<MemberInfo> propertiesToMap = candidateProperties.Where(p => modelInspector.IsPersistentProperty(p) && !modelInspector.IsPersistentId(p));
            HashSet<MemberInfo> propertiesInSplits = new HashSet<MemberInfo>();
            foreach (string splitGroup in splitGroups)
            {
                string groupId = splitGroup;
                List<MemberInfo> propertiesOfTheGroup = propertiesToMap.Where(p => modelInspector.IsTablePerClassSplit(type, groupId, p)).ToList();
        ...
        {
            JoinedSubclassMapper classMapper = new JoinedSubclassMapper(type, mapping);
            IEnumerable<MemberInfo> candidateProperties = null;
            if (!modelInspector.IsEntity(type.BaseType))
            {
                Type baseType = GetEntityBaseType(type);
        ...
        private void MapRootClass(Type type, HbmMapping mapping)
        {
            ...
            if (poidPropertyOrField != null && modelInspector.IsComponent(poidPropertyOrField.GetPropertyOrFieldType()))
            {
                classMapper.ComponentAsId(poidPropertyOrField, compoAsId =>
                                                               {
                                                                   PropertyPath memberPath = new PropertyPath(null, poidPropertyOrField);
                                                                   ComponentAsIdLikeComponentAttributesMapper componentMapper = new ComponentAsIdLikeComponentAttributesMapper(compoAsId);
                                                                   InvokeBeforeMapComponent(memberPath, componentMapper);

                                                                   Type componentType = poidPropertyOrField.GetPropertyOrFieldType();
            ...
            IEnumerable<string> splitGroups = modelInspector.GetPropertiesSplits(type);
            List<MemberInfo> propertiesToMap = persistentProperties.Except(naturalIdPropeties).Except(composedIdPropeties).Where(mi => !modelInspector.IsVersion(mi) && !modelInspector.IsVersion(mi.GetMemberFromDeclaringType())).ToList();
            HashSet<MemberInfo> propertiesInSplits = new HashSet<MemberInfo>();
            foreach (string splitGroup in splitGroups)
            {
                string groupId = splitGroup;
        ...
        private void MapSplitProperties(Type propertiesContainerType, IEnumerable<MemberInfo> propertiesToMap, IJoinMapper propertiesContainer)
        {
            foreach (MemberInfo property in propertiesToMap)
            {
                MemberInfo member = property;
                Type propertyType = property.GetPropertyOrFieldType();
                PropertyPath memberPath = new PropertyPath(null, member);
        ...
        private void InvokeAncestorsCustomizers(IEnumerable<Type> typeAncestors, IClassMapper classMapper)
        {
            ...
            foreach (Type entityType in typeAncestors.Where(t => !modelInspector.IsEntity(t)))
            {
        ...
        private void MapComposedIdProperties(IMinimalPlainPropertyContainerMapper composedIdMapper, PropertyPath propertyPath)
        {
            ...
            PropertyPath memberPath = propertyPath;
            if (modelInspector.IsProperty(member))
            {
                                   ...
                                   IPropertyContainerMapper propertiesContainer, PropertyPath path)
        {
            foreach (MemberInfo property in propertiesToMap)
            {
                MemberInfo member = property;
                Type propertyType = property.GetPropertyOrFieldType();
                PropertyPath memberPath = new PropertyPath(path, member);
                if (modelInspector.IsProperty(member))
                {
                    MapProperty(member, memberPath, propertiesContainer);
                }
        ...
        private void MapDynamicComponent(MemberInfo member, PropertyPath memberPath, Type propertyType, IPropertyContainerMapper propertiesContainer)
        {
            propertiesContainer.Component(member, (IDynamicComponentMapper componentMapper) =>
                                                  {
                                                      Type componentType = modelInspector.GetDynamicComponentTemplate(member);
        ...
        protected void ForEachMemberPath(MemberInfo member, PropertyPath progressivePath, Action<PropertyPath> invoke)
        {
            ...
            HashSet<PropertyPath> invokedPaths = new HashSet<PropertyPath>();
            ...
            {
                PropertyPath propertyPathInterfaceLevel = new PropertyPath(null, propertyOnInterface);
                invoke(propertyPathInterfaceLevel);
            }

            // path on declaring type
            PropertyPath propertyPathLevel0 = new PropertyPath(null, member.GetMemberFromDeclaringType());
            // path on reflected type
            PropertyPath propertyPathLevel1 = new PropertyPath(null, member);
        ...
        private void MapBag(MemberInfo member, PropertyPath propertyPath, Type propertyType, ICollectionPropertiesContainerMapper propertiesContainer,
                            Type propertiesContainerType)
        {
        ...
        private void MapList(MemberInfo member, PropertyPath propertyPath, Type propertyType, ICollectionPropertiesContainerMapper propertiesContainer,
                             Type propertiesContainerType)
        {
            Type collectionElementType = GetCollectionElementTypeOrThrow(propertiesContainerType, member, propertyType);
            ICollectionElementRelationMapper cert = DetermineCollectionElementRelationType(member, propertyPath, collectionElementType);
        ...
        private void MapDictionary(MemberInfo member, PropertyPath propertyPath, Type propertyType, ICollectionPropertiesContainerMapper propertiesContainer,
                                   Type propertiesContainerType)
        {
        ...
        private void MapSet(MemberInfo member, PropertyPath propertyPath, Type propertyType, ICollectionPropertiesContainerMapper propertiesContainer,
                            Type propertiesContainerType)
        {
            Type collectionElementType = GetCollectionElementTypeOrThrow(propertiesContainerType, member, propertyType);
            ICollectionElementRelationMapper cert = DetermineCollectionElementRelationType(member, propertyPath, collectionElementType);
        ...
        private void MapIdBag(MemberInfo member, PropertyPath propertyPath, Type propertyType, ICollectionPropertiesContainerMapper propertiesContainer,
                              Type propertiesContainerType)
        {
            Type collectionElementType = GetCollectionElementTypeOrThrow(propertiesContainerType, member, propertyType);
            ICollectionElementRelationMapper cert = DetermineCollectionElementRelationType(member, propertyPath, collectionElementType);
        ...
        {
            Type ownerType = property.ReflectedType;
            ...
            if (modelInspector.IsManyToMany(property))
            {
                ...
                {
                    Type[] args = property.GetPropertyOrFieldType().GetGenericArguments();

                    if (modelInspector.IsEntity(args.Last()))
                    {
        ...
        private class ComponentRelationMapper : ICollectionElementRelationMapper
        {
            ...
            public ComponentRelationMapper(MemberInfo collectionMember, Type ownerType, Type componentType, ICandidatePersistentMembersProvider membersProvider,
                        ...
                        propertiesContainer.Component(member, x =>
                                                              {
                                                                  ...
                                                                  Type componentOwnerType = type;
                                                                  Type componentPropertyType = propertyType;
        ...
        private class KeyComponentRelationMapper : IMapKeyRelationMapper
        {
            ...
            public KeyComponentRelationMapper(Type dictionaryKeyType, PropertyPath propertyPath, ICandidatePersistentMembersProvider membersProvider, IModelInspector domainInspector,
            ...
            private void MapProperties(IComponentMapKeyMapper propertiesContainer, IEnumerable<MemberInfo> persistentProperties)
            {
                foreach (MemberInfo property in persistentProperties)
                {
                    ...
                    if (domainInspector.IsManyToOne(member))
                    {
                        propertiesContainer.ManyToOne(member, manyToOneMapper =>
                                                              {
                                                                  PropertyPath progressivePath = new PropertyPath(propertyPath, member);
        ...
        private class OneToManyRelationMapper : ICollectionElementRelationMapper
        {
            ...
            public OneToManyRelationMapper(PropertyPath propertyPath, Type ownerType, Type collectionElementType, IModelInspector domainInspector,
                                           ICustomizersHolder customizersHolder, ModelMapper modelMapper)
            {
                this.propertyPath = propertyPath;
                this.ownerType = ownerType;
        ...
        {
            CustomizersHolder thisCustomizerHolder = customizerHolder as CustomizersHolder;
            ...
            CustomizersHolder otherCustomizerHolder = mapping.CustomizersHolder as CustomizersHolder;
        ...
        public void AddMapping(Type type)
        {
            ...
            IConformistHoldersProvider mapping = mappingInstance as IConformistHoldersProvider;
            if (mapping == null)
            {
        ...
        public void AddMappings(IEnumerable<Type> types)
        {
            ...
            foreach (Type type in types.Where(x => typeof(IConformistHoldersProvider).IsAssignableFrom(x) && !x.IsGenericTypeDefinition))
            {
                AddMapping(type);
            }
        }
